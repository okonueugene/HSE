...
class LostTimeAccidentsController extends Controller
{
  ...
  {
    if ($request->ajax()) {
      $query = Incident::where('incident_type_id', 4)
        ->with('media')
        ->orderBy('id', 'desc');
      ...
      return DataTables::of($query)
        ...
        <div class="btn-group">
          ...
          <ul class="dropdown-menu">
            <li><a href="javascript:void(0)" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#showModal"
                onclick="showDataModal(' .
            $row->id .
            ')">View</a>
            </li>
            <li><a href="javascript:void(0)" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#updateModal"
                data-lostTimeAccident-id="' .
            $row->id .
    ...
    $page_title = 'Lost Time Accidents';

    return view('admin/incidents/lost_time_accidents', compact('page_title'));
  }
